...
if(isset($_POST['add'])) {
    ...
    // Horaires stricts : 08h-12h OU 13h-17h
    ...
    if ($debut >= $fin) {
        ...
        $alert_msg = "<div class='alert alert-danger'>⛔ Horaires invalides ! Les cours BT doivent être entre <b>08h-12h</b> ou <b>13h-17h</b>.</div>";
    } else {
        ...
        if ($conflit) {
            if ($conflit['salle_id'] == $salle_id) {
                $alert_msg = "<div class='alert alert-danger'>⛔ SALLE OCCUPÉE par {$conflit['nom_complet']} ({$conflit['heure_debut']}-{$conflit['heure_fin']}).</div>";
            } else {
                $alert_msg = "<div class='alert alert-danger'>⛔ PROFESSEUR INDISPONIBLE ! Il est déjà en salle {$conflit['nom_salle']}.</div>";
            }
        } else {
            // Récupération infos filière
            ...
            $alert_msg = "<div class='alert alert-success'>✅ Cours BT planifié !</div>";
        }
    }
}

// Données (Filtre sur 'BT')
$profs = $pdo->query("SELECT * FROM professeurs WHERE categorie = 'BT'")->fetchAll();
$salles = $pdo->query("SELECT * FROM salles WHERE categorie = 'BT'")->fetchAll();
$filieres = $pdo->query("SELECT * FROM filieres WHERE categorie = 'BT'")->fetchAll();

// Liste des cours BT
$res = $pdo->query("SELECT r.*, p.nom_complet, s.nom_salle FROM reservations r JOIN professeurs p ON r.prof_id = p.id JOIN salles s ON r.salle_id = s.id WHERE r.niveau LIKE 'BT%' ORDER BY r.date_res DESC")->fetchAll();
?>

<div class="row">
    <div class="col-12">
        <h2 class="fw-bold" style="color: #e65100;"><i class="fas fa-tools"></i> Planning - Niveau BT</h2>
        <?= $alert_msg ?>
    </div>
    ...
    <div class="col-md-12 mb-4">
        <div class="card card-custom border-0 shadow-sm">
            <div class="card-header-custom" style="background: linear-gradient(135deg, #ff9966, #ff5e62);">
                <i class="fas fa-calendar-plus"></i> Nouveau Cours BT
            </div>
            <div class="card-body">
                <form method="post" class="row g-3">
                    <div class="col-md-4">
                        <label class="fw-bold small text-muted">PROFESSEUR</label>
                    ...
                    <div class="col-md-4">
                        <label class="fw-bold small text-muted">SALLE</label>
                    ...
                    <div class="col-md-4">
                        <label class="fw-bold small text-muted">FILIÈRE</label>
                    ...
                    <div class="col-md-2 d-flex align-items-end"><button type="submit" name="add" class="btn text-white w-100 fw-bold" style="background: #ff5e62;">Valider</button></div>
                </form>
            </div>
        </div>
    </div>
    ...
    <div class="col-md-12">
        <div class="card card-custom">
            <div class="card-header bg-white"><h5 class="m-0 text-secondary fw-bold">📅 Emploi du temps BT</h5></div>
                ...
                <table class="table table-hover mb-0 align-middle">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><span class="badge bg-warning text-dark"><?= $r['niveau'] ?></span></td>
